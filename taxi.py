...
async def logUserBot():
    load_dotenv()
    api_id = int(22917955)
    api_hash = "e9fafb00dc3f6c4e5b451acf2ff3c9f3"
    phone_number = "51934471704"
    ...
    await client.send_message("@spmtaxi", f'<b>Bot encendido</b>', parse_mode="HTML")
    spammer_group = int("-4578470160")
    ...
    while True:
        ...
        try:
            await client.send_message("@spmtaxi", f"<b>CANTIDAD DE MENSAJES CONSEGUIDOS PARA PUBLICAR</b> <code>{len(messages_list)-1}</code>",parse_mode="HTML")
        except:
            pass
                ...
                if i['group_name'] not in ["Spam 2024","RESPALDO🇵🇪BINS PERU","➳𝒀𝑨𝑷𝑬 𝑫𝑬 𝑬𝑺𝑻𝑨𝑭𝑨𝑫𝑶𝑹𝑬𝑺 ✧","QUEMANDO ESTAFADORES","𝐏𝐄𝐑Ú 𝐀𝐘𝐔𝐃𝐀","Referencias Elmer 💸","🎭 CANAL MUNDO STREAMING PERÚ 🇵🇪","TU MARKETPLACE"]:
                    ...
                    for message_spam in messages_list:
                        ...
                        try:
                            ...
                            await client.send_message("@spmtaxi", f'<b>Error enviando mensajes a {i["group_id"]}</b> - <code>{i["group_name"]}<code>\nCausa:{error}',parse_mode="HTML")
                            resultado = False
                        if resultado:
                            await client.send_message("@spmtaxi", f'<b>Mensaje enviado a {i["group_id"]}</b> - <code>{i["group_name"]}</code>',parse_mode="HTML")  
                        await asyncio.sleep(30)
                        if j==1: break
            await client.send_message("@spmtaxi", f'<b>RONDA ACABADA</b>', parse_mode="HTML")
